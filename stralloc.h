...
extern int stralloc_count_char(stralloc *s, const char c, unsigned int *level, unsigned int flags);
extern int stralloc_iconv(stralloc *sinput, stralloc *soutput, const char *from, const char *to);
...
*/
extern int stralloc_fd_cat(int fd, stralloc *s, uint32 size);

/* int stralloc_execute(stralloc *s, char **args, int uid, int gid);
 * Execute a command a concats its response into s
 * using provided uid and gid
 * if uid and gid are -1, set(gu)id are not used
 */
extern int stralloc_execute(stralloc *s, char **args, int uid, int gid);
...
extern int stralloc_replace(stralloc *str, const char *this, const char *that);
